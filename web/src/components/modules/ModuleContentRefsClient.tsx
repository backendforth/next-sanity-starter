...
import type { ModuleContentRefListItem } from "@/sanity/types/modules";
import {
	MediaImage,
...
function targetTitle(target: ModuleContentRefListItem): string {
...
function sortKey(target: ModuleContentRefListItem, locale: string): string {
	return targetTitle(target).toLocaleLowerCase(locale);
}
...
function collectCategoryOptions(
	...
	for (const item of items) {
		for (const cat of item?.categories ?? []) {
			...
			const label = typeof cat?.title === "string" ? cat.title.trim() : "";
...
export function ModuleContentRefsClient({
	...
	const categoryOptions = useMemo(() => {
		const options =
			isProjectList && showCategoryFilters ? collectCategoryOptions(items) : [];
		return options.sort((a, b) => a.label.localeCompare(b.label, locale));
	}, [isProjectList, showCategoryFilters, items, locale]);
	...
	const visibleItems = useMemo(() => {
		...
		if (sort === "az") {
			sorted.sort((a, b) =>
				sortKey(a, locale).localeCompare(sortKey(b, locale), locale),
			);
		} else {
			sorted.sort((a, b) => createdAtMs(b) - createdAtMs(a));
		}
		return sorted;
	}, [items, categoryId, sort, locale]);
